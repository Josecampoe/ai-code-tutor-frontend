        ...
        return (
          <div key={i} className="flex items-start flex-1">
            <div className="flex flex-col items-center cursor-pointer" onClick={() => onStepClick(i)}>
              ...
              <span className={`text-[10px] mt-1 max-w-[60px] text-center leading-tight ${isCurrent ? 'text-[#534AB7] font-medium' : 'text-[#9CA3AF]'}`}>
                {TYPE_LABEL[s.type] ?? 'Step'}
              </span>
            </div>
            {i < sections.length - 1 && (
              <div className={`flex-1 h-[1.5px] mt-[14px] mx-1 ${i < currentIndex ? 'bg-[#0F6E56]' : 'bg-[#E5E7EB]'}`} />
            )}
          </div>
        );
      })}
